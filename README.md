...
The generator is written the way you would describe the line out loud: straight for a bit,
then turn this way this hard, then that way, loosening. Each segment starts from the point
and heading the last one ended on, so the curve is continuous by construction. A `gold`
segment is a logarithmic spiral (radius multiplies by a fixed factor every quarter turn,
which is what makes a Fibonacci spiral a spiral); growth above 1 opens it out, below 1
winds it in. Useful fact when tuning: a `gold` segment's **exit heading is just its entry
heading plus its turn**, whatever the growth, so size the last one to finish pointing down
the page or the join into the descent shows as a corner.
